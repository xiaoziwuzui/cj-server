 ...
 * @access public
 * @param array|mixed $obj 对象或者数组
 * @author Xuan Yan
 * @return array|mixed 对象或者数组
 */
...
function dump($var, $exit = true){
    global $_F;
    $output = is_null($var) ? 'NULL' : print_r($var, true);
    if ($_F['run_in'] != 'shell') {
        echo "<html><head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, user-scalable=no, minimum-scale=1.0, maximum-scale=1.0\"><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"><title>Debug OutPut</title></head><body><div align=left><pre>" . htmlspecialchars($output) . "</pre></div></body></html>";
    } else {
        echo $output, chr(10);
    }
    if ($exit) exit(0);
