...
class FRedis
{
    ...
    /**
     * @var Redis $_REDIS 实例名
     */
    public $_REDIS = null;

    /**
     * @var Redis $_TRANSCATION 事物对象
     */
    ...
    public static function getInstance($db_name, $conf = 'redis_config')
    {
        global $_F;
        if ($_F['test_mode']) {
            $conf = str_replace('redis_','redis_test_',$conf);
        }
        if($_F['dev_mode']){
            $conf = str_replace('redis_','redis_local_',$conf);
        }
    ...
    public function ping()
    {
        ...
        return strpos($return,'PONG') ? true : false;
    }
     ...
     * @param string|int|bool $value 参数值
     * @return bool
     */
...
//        $value = json_encode($value);
     ...
     * @param $repeat int 判断key是否存在  0:不判断存在 1:判断存在 如果key存在，就不会有任何操作
     * @return bool
     */
     ...
     * @param null $list2 第二个队列名 默认null
     * @param int $timeout timeout为0:只获取list1队列的数据 timeout>0:如果队列list1为空 则等待timeout秒 如果还是未获取到数据 则对list2队列执行pop操作
...
//        }
    ...
    /**
     * 获取满足给定pattern的所有key
     * @param string $key regexp key匹配表达式 模式:user* 匹配以user开始的key
     * @return array|bool
     */
