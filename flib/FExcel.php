...
class FExcel
{
    ...
    /**
     * Set encoding
     * @param string $sEncoding type to set
     */
    ...
    private function addRow($array)
    {
        ...
        foreach ($array as $k => $v):
            $type = 'String';
            if ($this->bConvertTypes === true && strlen($v) <= 12 && is_numeric($v)):
                $type = 'Number';
            endif;
    ...
    public function saveXML($filepath,$append = false){
        $file = array();
        if(!is_file($filepath) || file_get_contents($filepath) == ''){
            $append = false;
        }
        if($append == false){
            $file[] = stripslashes(sprintf($this->header, $this->sEncoding));
            $file[] = "\n<Worksheet ss:Name=\"" . $this->sWorksheetTitle . "\">\n<Table>\n";
            foreach ($this->lines as $line){
                $file[] = $line;
            }
            ...
            $file[] = str_replace($this->footer,'',file_get_contents($filepath));
            $file[] = "\n<Worksheet ss:Name=\"" . $this->sWorksheetTitle . "\">\n<Table>\n";
            foreach ($this->lines as $line){
                $file[] = $line;
            }
