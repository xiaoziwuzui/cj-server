...
class FTable
{
    ...
    public function find($priValue = null)
    {
        ...
        if ($this->options['group_by']) {
            ...
            //修改 count 时候不要limit edit by wyr
...
//            $this->limit(1);
        ...
        if ($this->options['page']) {
            $this->setPagerOptions(
                array(
                    ...
                    'having_by' => $this->options['having_by']
                )
            );
        }
    ...
    private function buildSql()
    {
        ...
        if ($this->options['having_by'] && is_array($this->options['having_by'])) {
            $sql .= ' HAVING ' . implode(',', $this->options['having_by']);
        }
...
//            }
    ...
    public function save($data)
    {
        ...
        if ($this->options['where']) {
            // 更新
            $sql = "UPDATE {$this->table_raw} SET " . join(', ', $set) . " WHERE {$this->options['where']}";
            $params = array_merge($tempParams, $this->options['params']);
        } else {
            // 插入
            $sql = "INSERT INTO {$this->table_raw} SET " . join(', ', $set);
            $params = $tempParams;
        }
     ...
     * @return array|bool
     */
    public function getPagerInfo($count = 0)
    {
        global $_F;
