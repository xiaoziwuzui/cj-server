...
class Service_Tree
{
	 ...
	 * @return string
	 */
	public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '',$type = 1)
	{
		$number = 1;
		if($type == 1){
			$child = $this->get_child($myid);
		}else{
			$child = $this->get_parent($myid);
		}
		if (is_array($child)) {
			$total = count($child);
			$nstr = '';
			foreach ($child as $id => $value) {
				...
				$this->ret .= $nstr;
				$nbsp = $this->nbsp;
				$this->get_tree($value[$this->mid], $str, $sid, $adds . $k . $nbsp, $str_group,$type);
				$number++;
			}
		}
		return $this->ret;
