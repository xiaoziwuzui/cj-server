...
//        self::getGroupRole($gid);
    ...
    public static function AuthToken($token,$ip){
        ...
        $tokenInfo = FCache::get(self::$token_fix.$token);
        if(!$tokenInfo){
        ...
        FCache::set(self::$token_fix.$token,$cacheInfo,$cacheTime);
    ...
    public static function GenerateToken($userInfo,$ip){
        ...
        FCache::set(self::$token_fix.$token,array(
            'uid'         => $userInfo['uid'],
            'create_time' => time(),
            'expire_time' => time() + (86400 * 3),
            'ip'          => $ip,
    ...
    public static function DestroyToken($token){
        ...
        $uid = FCache::get(self::$token_fix.$token);
        if(!$uid){
            $table = new FTable('manager_token');
