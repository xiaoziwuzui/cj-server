...
class ccb extends PayAdapter {
    ...
    public function payOrder($param = array())
    {
        ...
        $this->config['return_url'] = 'http://'.$_F['http_host']. '/pay.html?order_id=' . $param['order_no'];
        ...
        $data = array(
            ...
            'CLIENTIP'   => FRequest::get_client_ip(),
            ...
            'SUB_OPENID' => escape($param['openid']),
            'CCB_IBSVersion' => 'V6',
        );
        ...
        if($result['SUCCESS'] == 'true'){
            $payInfo = $this->request->sendHttpGet($result['PAYURL']);
            FLogger::write($result,'ccb');
            FLogger::write($payInfo,'ccb');
            $payInfo = json_decode($payInfo,true);

            if($payInfo['ERRCODE'] == '000000'){
