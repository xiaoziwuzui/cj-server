...
class WeixinChat
{
    ...
    //发送模板消息
    const MESSAGE_TEMPLATE_SEND_URL = '/message/template/send?';
    ...
    //素材列表
    const batchget_material_url = '/material/batchget_material?';
    ...
    public function getRevImage()
    {
        if (isset($this->_receive['PicUrl'])) {
            return array(
                ...
                'mediaId' => $this->_receive['MediaId'] //图片消息用户id，可以调用多用户文件下载接口拉取数据。
            );
        }
        return false;
    }
    ...
    public function getRevVoice()
    {
        if (isset($this->_receive['MediaId'])) {
            return array(
                'mediaId' => $this->_receive['MediaId'], //语音消息用户id，可以调用多用户文件下载接口拉取数据。
    ...
    public function getRevVideo()
    {
        if (isset($this->_receive['MediaId'])) {
            return array(
                'mediaId' => $this->_receive['MediaId'], //视频消息用户id，可以调用多用户文件下载接口拉取数据。
                'thumbMediaId' => $this->_receive['ThumbMediaId'] //视频消息缩略图的用户id，可以调用多用户文件下载接口拉取数据。
            );
        }
        return false;
    }
    ...
    public function createMenu($data)
    {
        ...
        return $this->parserResult($result,'createMenu');
    }
    ...
    public function createMenuNew($data)
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->https_request(self::API_URL_PREFIX . self::MENU_CREATE_URL . 'access_token=' . $this->access_token, $this->jsonEncode($data));
        return $this->parserResult($result,'createMenuNew');
    }
    ...
    public function getMenu()
    {
        ...
        return $this->parserResult($result,'getMenu');
    }
    ...
    public function deleteMenu()
    {
        ...
        return $this->parserResult($result,'deleteMenu');
    }
    ...
    public function getUserInfo($openid)
    {
        ...
        return $this->parserResult($result,'getUserInfo');
    }
    ...
    public function getUserList($next_openid = '')
    {
        ...
        return $this->parserResult($result,'getUserList');
    }
    ...
    public function getGroup()
    {
        ...
        return $this->parserResult($result,'getGroup');
    }
    ...
    public function batchget_material($type = 'image',$offset = 0,$count = 20)
    {
        ...
        $result = $this->https_request(self::API_URL_PREFIX . self::batchget_material_url . 'access_token=' . $this->access_token,$data);
        return $this->parserResult($result,'batchget_material');
    }
    ...
    public function createGroup($name)
    {
        ...
        return $this->parserResult($result,'createGroup');
    }
    ...
    public function updateGroup($id, $name)
    {
        ...
        return $this->parserResult($result,'updateGroup');
    }
    ...
    public function updateGroupMembers($openid, $to_groupid)
    {
        ...
        return $this->parserResult($result,'updateGroupMembers');
    }
    ...
    public function sendCustomMessage($touser, $data, $msgType = 'text')
    {
        ...
        switch ($msgType) {
            ...
            case 'video': // 发送视频消息
                $arr['video']['media_id'] = $data['media_id']; // 发送的视频的用户ID
                $arr['video']['thumb_media_id'] = $data['thumb_media_id']; // 视频缩略图的用户ID
                break;
            ...
            case 'music': // 发送音乐消息
                ...
                $arr['music']['thumb_media_id'] = $data['title']; // 缩略图的用户ID
                break;
        ...
        return $this->parserResult($result,'sendCustomMessage');
    }
    ...
    public function upload_media($type, $file)
    {
        ...
        $result = $this->https_request_media($url, $data);
        return $this->parserResult($result,'upload_media');
    }

    /**
     * 上传图文消息素材
     * @param array $data 中的 media_id 在upload_media上传中得到的media_id
     * @return bool|string
     */
    public function uploadnews_media($data)
    {
        ...
        return $this->parserResult($result,'uploadnews_media');
    }
    ...
    public function send_massMsg($toUserList, $media_id)
    {
        ...
        return $this->parserResult($result,'send_massMsg');
    }
     ...
     * @param int $scope 应用授权作用域 0为snsapi_base，1为snsapi_userinfo
    ...
    public function getSnsAccessToken($code)
    {
        ...
        return $this->parserResult($result,'getSnsAccessToken');
    }

    public function getSnsAccess1Token($code)
    {
        $result = $this->https_request(self::SNS_OAUTH_ACCESS_TOKEN_URL . 'appid=' . $this->appid . '&secret=' . $this->appsecret . '&code=' . $code . '&grant_type=authorization_code');
        return $this->parserResult($result,'getSnsAccess1Token');
    ...
    public function refershToken($refresh_token)
    {
        ...
        return $this->parserResult($result,'refershToken');
    }
    ...
    public function getSnsUserInfo($access_token, $openid)
    {
        ...
        return $this->parserResult($result,'getSnsUserInfo');
    }
    ...
    public function getUnionUserInfo($access_token, $openid)
    {
        ...
        return $this->parserResult($result,'getUnionUserInfo');
    }
     ...
     * @param int $expire 该二维码有效时间，以秒为单位。 最大不超过 30 天。
     * @return bool|string
     */
    public function createQrcode($scene_id, $type = 0, $expire = 1800)
    {
        ...
        return $this->parserResult($result,'createQrcode');
    }
    ...
    public function createQrcodeByScene_str($scene_str)
    {
        ...
        $result = $this->https_request(self::API_URL_PREFIX . self::QRCODE_CREATE_URL . 'access_token=' . $this->access_token, $this->jsonEncode($data));
        return $this->parserResult($result,'createQrcodeByScene_str');
    }
    ...
    public function error($data)
    {
        ...
        FLogger::write('WEIXIN API errcode:[' . $this->errCode . '] errmsg:[' . $this->errMsg . ']','wechat_error');
    ...
    //https请求（支持GET和POST）
    public function https_request_media($url, $data = null)
    {
        $file = new CURLFile($data['media']);
    ...
    public function getJsSDKConfig($c_url)
    {
        ...
        if (!$ticket) {
            ...
            if ($result) {
                ...
                FCache::set($cacheKey, $ticket, $expire);
            }
        }
    ...
    public function getcallbackip()
    {
        ...
        return $this->parserResult($result,'getcallbackip');
    }
     ...
     * @return bool
     */
    public function sendTemplateMessage($openID,$template_id, $data,$url = '')
    {
        $arr = array(
            'touser'      => $openID,
...
//        }
        ...
        $result = $this->https_request(self::API_URL_PREFIX . self::MESSAGE_TEMPLATE_SEND_URL . 'access_token=' . $this->access_token, $this->jsonEncode($arr));
        return $this->parserResult($result,'sendTemplateMessage');
    }
     ...
     * @return array|bool
     */
    public function parserResult($json = '',$method = ''){
        if ($json) {
