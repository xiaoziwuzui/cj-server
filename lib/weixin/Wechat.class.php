...
class Wechat
{
     ...
     * @return string
     */
    private function checkSignature($string = '')
    {
        ...
        $token = $this->token;
        $tmpArr = array($token, $timestamp, $nonce,$string);
     ...
     * @return bool|string orderId|boolean
     */
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @param int $expired
     */
    protected function setCache($key,$value,$expired){
        FCache::set($key,$value,$expired);
    }
     ...
     * @return boolean|string 签名值
     */
    public function getTicketSignature($data,$method="sha1") {
        if (!function_exists($method)) return false;
        $newArray = array();
    ...
    /**
     * 获取微信服务器IP地址列表
     * @return bool|array array('127.0.0.1','127.0.0.1')
     */
    ...
    /**
     * 获取菜单(认证后的订阅号可用)
     * @return bool|array array('menu'=>array(....s))
     */
     ...
     * @return bool|array array('button'=>array(....s))
     */
     ...
     * @return bool|array array('ticket'=>'qrcode字串','expire_seconds'=>604800,'url'=>'二维码图片解析后的地址')
     */
     ...
     * @return string
     */
    public function getOauthRedirect($callback,$scope = 0,$state = ''){
     ...
     * @return array|boolean
     */
    public function getPoiCate() {
        if (!$this->access_token && !$this->checkAuth()) {
            return false;
        }
        $url = self::API_URL_PREFIX . '/poi/getwxcategory?access_token=' . $this->access_token;
     ...
     * @param array $data 具体结构请参看卡券开发文档(6.1.2 会员卡交易)章节
     * @return boolean|array
     */
     ...
     * @param string $method
     */
    public function error($data,$method = '')
    {
        $this->errCode = $data['errcode'];
        $this->errMsg  = $data['errmsg'];
        FLogger::write('errcode:[' . $this->errCode . '] errmsg:[' . $this->errMsg . ']' . ($method != '' ? ' method:[' . $method . ']' : ''),'wechat_error');
        echo('errcode:[' . $this->errCode . '] errmsg:[' . $this->errMsg . ']');
    }
