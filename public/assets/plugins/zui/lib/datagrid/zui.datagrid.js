...
(function($, undefined) {
    ...
    var DataGrid = function(element, options) {
        ...
        $container.on('touchstart', function(e) {
            that.touchXY.startX = e.originalEvent.changedTouches[0].clientX;
            that.touchXY.startY = e.originalEvent.changedTouches[0].clientY;
            $container.on('touchend', function(e) {
                that.touchXY.startX = 0;
                that.touchXY.startY = 0;
                $container.off('touchmove');
                $container.off('touchend');
            });
            $container.on('touchmove', function(e) {
                var localX = e.originalEvent.changedTouches[0].clientX;
                var x = 0;
                var y = 0;
                x = that.touchXY.startX - localX;
                x = that.layout.scrollLeft - Math.round(x * mouseWheelFactor);
                that.scroll(x,0);
                // event.preventDefault();
            });
        });
        ...
        if (dataSource.array) {
                        ...
                        success: function(responseData, textStatus, jqXHR) {
                            ...
                            if ($.isPlainObject(responseData) && responseData.data) {
                                ...
                                if (result === 'success' || result === 'ok' || result === 200) {
                                    ...
                                    onFinish(false, responseData.msg || responseData.reason || that.lang['errorCannotHandleRemoteData'], responseData);
                                }
                            } else {
            ...
            if (loadData) {
                ...
                loadData(params, function(resultData, error) {
                    ...
                    if(that.options.showFooter && typeof(resultData[that.options.footerField]) !== 'undefined'){
                        that.resetData(dataId, resultData.data, resultData.pager,resultData[that.options.footerField]);
                    }else{
    ...
    DataGrid.prototype.renderLoading = function(loading) {
        ...
        if (loading) {
            if (!$loading.length) {
                $loading = $('<div class="datagrid-loading" style="display: none"><div class="content"><i class="icon icon-spin icon-spinner icon-2x"></i><div class="datagrid-loading-message"></div></div></div>').appendTo(that.$container);
            }
    ...
    DataGrid.prototype.getFooter = function() {
        var dataSource = this.dataSource;
        var data = null;
        if (typeof(dataSource.footer) !== 'undefined') {
            data = dataSource.footer;
        }
        return data;
    };

    DataGrid.prototype.resetData = function(dataId, data, pager,footer) {
        var dataSource = this.dataSource;
        dataSource.dataId = dataId;
    ...
    DataGrid.prototype.updateLayout = function() {
        ...
        if (containerHeight === 'auto') {
            containerHeight = layout.headerHeight + layout.rowsLength * (layout.rowHeight + layout.borderWidth);
        }
    ...
    DataGrid.prototype.getCell = function(rowIndex, colIndex,isFooter) {
        var that = this;
        var config = that.getCellConfig(rowIndex, colIndex,isFooter);
    ...
    DataGrid.prototype.getRowConfig = function(rowIndex,isFooter) {
        var that   = this;
        ...
        if(isFooter){
            dataItem = that.getFooter();
        }else{
            dataItem = rowIndex > 0 ? that.getDataItem(rowIndex - 1) : null;
        }
        config.data = dataItem;
        rowId = dataItem && (dataItem.rowId || dataItem.id);
    ...
    DataGrid.prototype.getColConfig = function(colIndex,isFooter) {
        var that = this;
        ...
        if (!config) {
            config = $.extend(
                ...
                colIndex > 0 ? (isFooter ? that.options.footerCols[colIndex-1] : that.dataSource.cols[colIndex - 1]) : null,
    ...
    DataGrid.prototype.getCellConfig = function(rowIndex, colIndex,isFooter) {
        var that = this;
        ...
        if (!config) {
            config = $.extend(
                {id: cellId},
                that.getColConfig(colIndex,isFooter),
                that.getRowConfig(rowIndex,isFooter),
    ...
    DataGrid.prototype.renderCell = function(rowIndex, colIndex, $row,isFooter) {
        isFooter = isFooter || false;
        var that       = this;
        var options    = that.options;
        var cell       = that.getCell(rowIndex, colIndex,isFooter);
    ...
    DataGrid.prototype.renderFooter = function(rowIndex) {
        var that       = this;
        var layout     = that.layout;
        var options    = that.options;
        var rowLayout  = that.getRowLayout(rowIndex);
        ...
        var $row       = $('#' + elementId);
        if (!$row.length) {
            $row = $('<div class="datagrid-row" />').appendTo(that.$cells);
            $row.attr({
                id: elementId,
                'data-row': rowIndex,
                'data-id': rowIndex
        ...
        for (var i = 0; i < colsLength; ++i) {
            that.renderCell(rowIndex, i, $row,'footer');
        }
        return $row;
    };
    ...
    DataGrid.prototype.renderData = function() {
        ...
        if (that.options.showFooter) {
            that.renderFooter(endRenderRow + 1);
        }
